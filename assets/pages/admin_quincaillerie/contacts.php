<?php 
    ...
    // liste des filieres
    $checkDeprt = $obj->Select('contact', [], [], $orderBy = '', $order = 1);
    ...
    if(isset($_POST['delete_department'])){
        extract($_POST);
        $requete = $obj->Delete('contact', array('ID_CONTACT'=>$idms));
        if($requete)
        {
                                ...
                                <tr>
                                    <td class="text-truncate">
                                        <div class="btn-group" role="group" aria-label="Button group">
                                            <button class="btn text-truncate" data-toggle="modal" data-target="#upModal" onclick="updatemessage(<?= $depListe['ID_CONTACT']?>)"><i class="fas fa-edit text-warning"></i></button>
                                            <button class="btn text-truncate" data-toggle="modal" data-target="#delModal"><i class="fas fa-trash text-danger" onclick="deletemessage(<?= htmlspecialchars(json_encode($depListe['ID_CONTACT']))?>,<?= htmlspecialchars(json_encode($depListe['NOM_COMPLET']))?>)"></i></button>
                                        </div>
                                    </td>
                                    <td class="text-truncate"><?= $depListe['NOM_COMPLET'] ?></td>
                                    <td class="text-truncate"><?= $depListe['EMAIL'] ?></td>
                                                ...
                                                <form action="#" method="post" id="depart" class="my-3">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-lg-4">
                                                            ...
                                                            <div class="input-group">
                                                                <input type="text" class="form-control" name="email" id="email" placeholder="Entrez votre motif" oninput="EnableDecimal(this)" required>
...
<script type="text/javascript">
    ...
    function EnableDecimal(val1){
        if(val1.id=="auteur_name" || val1.id=="motif"|| val1.id=="email"  || val1.id=="message" || val1.id=="up_msgs" || val1.id=="up_aut_name" || val1.id=="up_msg"){
            if (val1.value =="")
            {
