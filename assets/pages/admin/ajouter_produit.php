...
if(isset($_POST['submit_materiel']) AND $_POST['submit_materiel']!=empty($_POST['submit_materiel'])){
 //Recover input
   ...
   // Case of empty input
    ...
    {
        alert('danger',' Please fill all input !');
    }else
    {
      // Selected certains data from data base
      $select_product=$bdd->prepare("SELECT ID_QUINCAILLERIE, NOM_MATERIEL FROM materiel WHERE ID_QUINCAILLERIE=? OR NOM_MATERIEL=?");
      $select_product->execute([$matricule_quin,$nom_prod]);

      // count number of row of selected data and verifier in there is more than one row
       $count_mater=$select_product->rowCount();
       if($count_mater>0)
       {
         alert('danger', $nom_prod.' or '.$matricule_quin.' already exists');
       }
       else
       {
        // If this is not constraint, then insert input data
         ...
         alert('success',$nom_prod.' Successful added');
       } 
    }    
}
?>
